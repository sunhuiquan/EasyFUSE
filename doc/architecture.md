...
1. 实现简单的日志机制，保证数据一致性，遇到软件/物理错误导致系统崩溃后，可以根据磁盘日志块上的数据够恢复一致性，还有向写文件系统调用提供事务批处理提交的功能。
2. to do 原理图与日志恢复的原理
...
3. 提供磁盘上的 disk_inode 结构在内存上的 cache 机制，通过 cache 机制减少读写磁盘的加载的次数，提高效率，里面通过二层锁机制（对cache整体和每个内存中的inode分别加锁）实现并发性，通过引用计数来实现缓存里面的元素的释放与复用。
