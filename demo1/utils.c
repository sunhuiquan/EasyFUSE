...
int get_attr_from_open_pathblock(const char * path,
	...
	while (offset < blk_info->size) {			//在文件/目录的上级目录中找文件/目录
		...
		if (file_dir->flag != 0 && strcmp(file_dir->fname, p) == 0
				&& (q == NULL || strcmp(file_dir->fext, q) == 0)) { //find!
			//进入文件/目录所在块
...
int utils_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
	...
	if (get_attr_from_open_pathblock(path, attr) == -1) { //打开path指定的文件，将文件属性读到attr中
		freePtrs(attr, blk_info, NULL);
		return -ENOENT;
	}

	long start_blk = attr->nStartBlock;
	if (attr->flag == 1) {   //regual file
		freePtrs(attr, blk_info, NULL);
		return -ENOENT;
	}
	if (get_blkinfo_from_read_blkpos(start_blk, blk_info)) {   //assign blk_info
		freePtrs(attr, blk_info, NULL);
		return -ENOENT;
	}
	...
	while (pos < blk_info->size) {
		...
		if (file_dir->flag != 0 && name[strlen(name) - 1] != '~'
				&& filler(buf, name, NULL, 0))  //add filename into buf
			break;
		file_dir++;
...
int dv_path(char*name, char*ext, const char*path, long*p_dir_blk, int flag) {
	...
	if (flag == 2 && q != NULL)
		...
		*q = '\0';	//截断tmp_path，tmp_path=目录static
		...
		p = q;      //p=子目录 或 文件
		fslog("dvpath", "%s81\n", tmp_path);
		if (get_attr_from_open_pathblock(tmp_path, attr) == -1) { //按路径读取这一目录，获取目录属性，确认目录有效
			fslog("dvpath", "%s83\n", tmp_path);
			free(attr);
			return -ENOENT;
		}
	...
	if (q == NULL) { // /文件 或 /目录
		if (get_attr_from_open_pathblock("/", attr) == -1) { //读取根目录属性，确认根目录有效
			fslog("dvpath", "%s90\n", path);
			free(attr);
			return -ENOENT;
		}
...
int is_empty(const char* path) {
	...
	if (get_blkinfo_from_read_blkpos(start_blk, blk_info) == -1) {     //读出块信息
		freePtrs(blk_info, attr, NULL);
		return 0;
	}
...
int utils_setattr(const char* path, struct u_fs_file_directory* attr) {
	...
	while (offset < blk_info->size) {
		...
		if (file_dir->flag != 0 && strcmp(p, file_dir->fname) == 0
				&& (*q == '\0' || strcmp(q, file_dir->fext) == 0)) {     //找到该文件
			//设置为attr指定的属性
